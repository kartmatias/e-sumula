...
import HeaderModalidade from './HeaderModalidade.jsx';
import LinhaListagemModalidade from './LinhaListagemModalidade.jsx';
import {
	Button,
	Glyphicon
...
class ListagemModalidade extends Component {
	constructor(props) {
		super(props);
		this.state = {
			registros: {},
	...
	render() {
		...
		if (this.state.registros._embedded){
			listagemRegistros = this.state.registros._embedded.modalidade.map((modalidade) => {
				return <LinhaListagemModalidade key={modalidade.id} modalidade={modalidade} onRegistroSelect={this.onRegistroSelect} onRegistroDeselect={this.onRegistroDeselect}/>;
			});
		}
		let header = <HeaderModalidade/>;
		return (
			<div>
...
export default ListagemModalidade;